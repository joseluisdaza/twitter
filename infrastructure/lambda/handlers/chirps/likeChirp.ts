...
export const handler = async (event: APIGatewayProxyEventV2): Promise<APIGatewayProxyResultV2> => {
  try {
    ...
    await ddb.send(new PutCommand({
      ...
      Item: { chirpId, userId, createdAt: now },
      ConditionExpression: 'attribute_not_exists(chirpId)',
    }));
    ...
    if (error.name === 'ConditionalCheckFailedException') return noContent(); // already liked
    console.error('LikeChirp error:', err);
